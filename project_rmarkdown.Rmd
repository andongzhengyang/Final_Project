...
summary_tbl <- 
  list("Cost" = 
       list("min" = ~ min(COST), 
            "max" = ~ max(COST),
            "mean (sd)" = ~ qwraps2::mean_sd(COST)),
       "Wait Time" = 
       list("min" = ~ min(WAIT_TIME), 
            "max" = ~ max(WAIT_TIME),
            "mean (sd)" = ~ qwraps2::mean_sd(WAIT_TIME)),
       "Total Duration" = 
       list("min" = ~ min(TOTAL_DURATION), 
            "max" = ~ max(TOTAL_DURATION),
            "mean (sd)" = ~ qwraps2::mean_sd(TOTAL_DURATION)),
       "Day of Week" = 
       list("Monday" = ~ qwraps2::n_perc0(DAY_OF_WK == "Monday"),
            "Tuesday"  = ~ qwraps2::n_perc0(DAY_OF_WK == "Tuesday"),
            "Wednesday"  = ~ qwraps2::n_perc0(DAY_OF_WK == "Wednesday"))
       ) 

summary_table(data2,summary_tbl)
